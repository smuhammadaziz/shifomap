    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="mt-2 flex items-center gap-2 flex-wrap">
            <div className="text-gray-700 font-medium">{booking.doctorName || '—'} · {booking.serviceTitle || '—'}</div>
        ...
        <div className="flex gap-2 flex-wrap justify-end">
          ...
          {booking.status === 'confirmed' && <Button variant="secondary" onClick={() => action('patient-arrived')}>{bk.arrived ?? 'Keldi'}</Button>}
      ...
      <Card>
        ...
        <CardContent className="space-y-2 text-sm text-gray-700">
          ...
          <div><span className="text-gray-500">{bk.service ?? 'Xizmat'}:</span> {booking.serviceTitle || '—'}</div>
